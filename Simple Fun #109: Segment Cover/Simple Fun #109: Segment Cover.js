...
Task
Given some points(array A) on the same line, determine the minimum number of line segments with length L needed to cover all of 
the given points. A point is covered if it is located inside some segment or on its bounds.
...
function segmentCover(A, L) {
  if (A.length === 0) {
    ...
    let minA = Math.min(...A);
    let filteredA = A.filter((i) => i > minA + L);
    return 1 + segmentCover(filteredA, L);
  }
}